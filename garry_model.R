# my best attempt using risas original transcription and combining temperature response
...
model <- function(PAR = 800, temp_c = 25, CO2 = 200, O2 = 209, Abs = 0.85, beta = 0.52, 
  ...
  C <- CO2 / 1e6      # Partial pressure of CO2, ubar
  O <- O2 / 1e3       # Partial pressure of O2, mbar
  ...
  # Biochemical constants at 25 degrees C
  kq25 <- 300           # Cyt b6f kcat for PQH2 at 25 degrees C, mol e-1 mol sites-1 s-1
  nl25 <- 0.75          # ATP per e- in linear flow, ATP/e-
  nc25 <- 1             # ATP per e- in cyclic flow, ATP/e-
  kc25 <- 3.6           # Rubisco kcat for CO2 at 25 degrees C, mol CO2 mol sites-1 s-1
  ko25 <- 3.6 * 0.27    # Rubisco kcat for O2 at 25 degrees C, mol O2 mol sites-1 s-1
  KC25 <- 260 / 1e6     # Rubisco Km for CO2 at 25 degrees C, bar
  KO25 <- 179000 / 1e6  # Rubisco Km for O2 at 25 degrees C, bar
  
  # Add c -> k as well as gas constant
  temp_k <- temp_c + 273.15  # turning celcius into kelvin, K
  ...
  nl <- nl25 * (1 + exp((tref_k * En - Hd) / (R * tref_k))) / 
    (1 + exp((temp_k * En - Hd) / (R * temp_k)))    # ATP per e- in linear flow
  nc <- nc25 * (1 + exp((tref_k * En - Hd) / (R * tref_k))) / 
    (1 + exp((temp_k * En - Hd) / (R * temp_k)))    # ATP per e- in cyclic flow
  
  # Activation Energies
  ha_kq <- 37     # Activation energy of Jmax, kJ mol-1
  ha_rd <- 66     # Activation energy of mitochondrial respiration, kJ mol -1
  ha_S  <- 23     # Activation energy of Rubisco specificity, kJ mol -1
  ha_KC <- 59     # Activation energy of Rubisco Km for CO2, kJ mol -1
  ha_KO <- 36     # Activation energy of Rubisco Km for O2, kJ mol -1
  ha_kc <- 58     # Activation energy of Rubisco Kcat for CO2, kJ mol -1
  
  # Temperature dependent cyt b6f  equations
  kq <- kq25 * exp(ha_kq / R * (1 / tref_k - 1/temp_k))  # Cyt b6f kcat for PQH2, s-1
  Vqmax <- CB6F * kq                                   # maximum Cyt b6f activity, mol e-1 m-2 s-1
  
  # Temperature dependent rubisco equations
  KC <- KC25 * exp(ha_KC / R * (1 / tref_k - 1 / temp_k))   # Rubisco Km for CO2, bar
  KO <- KO25 * exp(ha_KO / R * (1 / tref_k - 1 / temp_k))   # Rubisco Km for O2, bar
  kc <- kc25 * exp(ha_kc / R * (1 / tref_k - 1 / temp_k))   # Rubisco kcat for CO2, s-1
  S25 <- (kc25 / KC25) * (KO25 / ko25)                      # Rubisco specificity for CO2/O2 at 25 degrees C, dimensionless
  S <- S25 * exp(ha_S / R * (1 / tref_k - 1 / temp_k))     # Rubisco specificity for CO2/O2, dimensionless
  Vcmax <- RUB * kc                                         # maximum Rubisco activity, mol CO2 m-2 s-1  
    
  #  Respiration and Other derived variables
  Rd25 <- RUB * Rds * kc25                                                       # mitochondrial respiration at 25 degrees C, mol CO2 m-2 s-1
  Rd <- Rd25 * exp(ha_rd / R * (1 / tref_k - 1 / temp_k))                        # mitochondrial respiration at 25 degrees C, mol CO2 m-2 s-1
  ...
  results <- data.frame("PAR"       = PAR,        # PAR, umol PPFD m-2 s-1
                        "Temp"      = temp_c,     # Leaf temperature, C
